...
const normalizeContentPart = (part: unknown): ContentPart => {
  if (typeof part === "string") return { type: "text", text: part };
  const p = part as ContentPart;
  if (p.type === "text") return p;
  if (p.type === "image_url") return p;
  if (p.type === "file_url") return p;
...
const normalizeToolChoice = (toolChoice: any, tools: any[] | undefined) => {
...
const fetchWithBackoff = async (url: string, init: RequestInit) => {
